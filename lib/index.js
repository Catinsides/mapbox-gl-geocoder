 ...
 * @param {Function} [options.externalGeocoder] A function like localGeocoder option but asynchronous, return a Promise results with GeoJson array
 ...
 * @param {Boolean} [options.externalGeocoderOnly=false] same as `localGeocoderOnly`
 * @example
...
MapboxGeocoder.prototype = {
  ...
  onAdd: function(map) {
    ...
    if (this.options.externalGeocoderOnly && !this.options.externalGeocoder) {
      throw new Error("A externalGeocoder function must be specified to use externalGeocoderOnly mode")
    }
  ...
  _geocode: function(searchInput) {
    ...
    var request;
    if (this.options.localGeocoderOnly || this.options.externalGeocoderOnly) {
    ...
    if (this.options.externalGeocoder) {
      request = Promise.all([request, this.options.externalGeocoder(searchInput)]);
    }

    request.then(
      function(response) {
        if (this.options.externalGeocoder) {
          if (Array.isArray(response) && response[0] && Array.isArray(response[1])) {
            var req = response[0];
            externalGeocoderRes = response[1];
            response = req;
          } else {
            ...
            this._renderError();
            this._eventEmitter.emit('results', { features: [] });
            this._eventEmitter.emit('error', { error: {
              msg: 'response is unvalid.',
              response: response
            }});
            return;
          }
        ...
        res.features = res.features
          ? externalGeocoderRes.concat(res.features)
          : externalGeocoderRes;
